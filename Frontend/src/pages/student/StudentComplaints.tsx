...
const StudentComplaints = () => {
  ...
  const filtered = complaints.filter((c) => {
    const matchSearch = c.title.toLowerCase().includes(search.toLowerCase()) || (c.id && c.id.toLowerCase().includes(search.toLowerCase()));
    const matchStatus = filterStatus === "all" || c.status === filterStatus;
    const matchStatusBackend = filterStatus === "all" || (c.status.toLowerCase() === filterStatus.toLowerCase().replace(" ", "_"));
    return matchSearch && (matchStatus || matchStatusBackend);
  });
